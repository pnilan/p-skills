...
- **Support level**: Read `metadata.yaml` and extract `data.supportLevel` (typically `certified` or `community`). This is the DR connector's certification status.
...
3. Create a GitHub issue in airbytehq/airbyte-internal-issues for DR connector gaps (P0/P1 items where the DR connector needs to catch up to the ADP connector)
```

**Option 3 details**: If the user selects option 3, create a single GitHub issue in `airbytehq/airbyte-internal-issues` using `gh issue create`. The issue should:
- **Title**: `[Parity] source-{connector_name}: DR connector gaps vs ADP connector`
- **Focus exclusively on DR gaps**: Only include items where the DR connector is missing functionality that the ADP connector has. Do NOT include gaps in the ADP connector direction — that is a separate concern.
- **Include the DR support level** (certified/community) in the issue body for context
- **Body structure**:
  ```markdown
  ## DR Connector Parity Gaps: `source-{connector_name}`

  **DR Connector**: `source-{connector_name}` (support level: `{certified|community}`)
  **ADP Connector**: `{connector_name}`
  **Audit Date**: {date}

  The following gaps were identified where the DR connector lacks functionality present in the ADP connector.

  ### P0 — Critical Gaps
  {critical gaps from the audit where DR is behind ADP}

  ### P1 — Important Gaps
  {important gaps from the audit where DR is behind ADP}

  ---
  _Generated by audit-connector-parity skill_
  ```
- Omit P2 items from the issue (minor gaps don't warrant tracking)
- After creating the issue, display the issue URL to the user
